...
window.addEventListener('message', function(event) {
    ...
    if (data.action === "OpenCreateMenu") {
      ...
      if (PartyLists[Party].started === true){
        ...
        hide(".startedloading")
        $(".section-main .join-container").append(`<button class="create" onclick="joinparty()">Gir</button>`)
      }
      ...
      $(".section-HUD #laps").html((AllLaps - Lap + 1)+"/"+AllLaps+"<i><a>Tur</a></i>")
      ...
      if (SpecPlayerName != "nil"){
        $(".section-spectate h2").html('Seyirci: '+SpecPlayerName)
        $(".section-spectate h3").html('Ok tuşlarıyla üyeler arasında geçiş yapabilirsiniz ⬅️ ➡️')
      }
      else{
...
function refreshjoinedplayer(){
  ...
  for (let i = 0; i < PartyLists[Party].members.length; i++) {
    if (PartyLists[Party].members[i]["ID"] === MyServerID){
      $(".section-main .join-container .create").remove()
      $(".section-main .join-container").append(`<button class="create" onclick="leaveparty()">Ayrılmak</button>`)
      break
    }
    else{
      $(".section-main .join-container .create").remove()
      $(".section-main .join-container").append(`<button class="create" onclick="joinparty()">Katıl</button>`)
    }
  }
  if (PartyLists[Party].started === true){
    show(".startedloading")
...
function cancelparty(){
  $(".sure .container h1").html("Bu partiyi iptal edeceğinden emin misin?")
  show(".sure")
  suredata = "cancelparty"
}

function startparty(){
  $(".sure .container h1").html("Bu partiyi başlatacağınızdan emin misiniz?")
  show(".sure")
  suredata = "startparty"
}
...
function leaveparty(){
  $(".sure .container h1").html("Bu partiden ayrılacağınıza emin misiniz?")
  show(".sure")
  suredata = "leaveparty"
}
